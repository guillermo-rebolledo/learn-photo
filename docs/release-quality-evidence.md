...
This document maps issue #16’s release bar to repeatable evidence. Automated results are release gates; physical-device and assistive-technology checks remain named manual gates and must not be inferred from emulation.
...
| Immediate preview; deferred refinement | The Sandbox exposes `data-render-quality="preview"` immediately with its 480 px derivative, waits 180 ms without further input, then mounts the active 960 px image, calibrated layers, and pixel-derived Histogram. Route tests assert that sequence. |
| 16.7 ms reference frame | `tests/performance.spec.ts` measures eight consecutive browser-visible Exposure Control updates from change dispatch to the Rendered Result mutation and rejects any sample over 16.7 ms. Expensive canvas and layer work starts only after the idle boundary. |
| LCP ≤2.5 s, INP ≤200 ms, CLS ≤0.1 | Buffered browser performance entries on `/sandbox` are asserted against all three thresholds. CI runs the route budget file independently. These are lab safeguards; field percentile reporting remains deferred with non-blocking analytics. |
...
| Browser smoke coverage | `playwright.smoke.config.ts` defines current Chrome, Edge, Firefox, Safari/WebKit, Android Chrome emulation, and iOS Safari emulation projects. CI runs each project separately, with branded Chrome and Edge channels on hosted runners. |
...
The local 2026-07-19 implementation run passed branded Chrome, Firefox, WebKit desktop, Android Chrome emulation, and iOS Safari emulation. Branded Edge was not installed on the implementation Mac and is therefore delegated to the Windows CI project rather than claimed as locally verified.
