...
export function LessonFour({ explanation }: { explanation: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    localStorage.setItem(storageKey, JSON.stringify({ ...saved, lesson: lessonFour.slug, lessonFourSettings: settings, lessonFourIntention: intention, completedChallenges: [...completedChallenges] }));
  }, [complete, hydrated, intention, settings]);
  ...
  return <div className={visualEffectsAvailable ? undefined : "no-visual-effects"}>
    ...
    <section className="experiment" aria-labelledby="cyclist-challenge-title">
      ...
      <div className="simulator cyclist-simulator">
        <CyclistPreview shutter={settings.shutter} motion={challengeMotion} label={`f/${settings.aperture} · 1/${settings.shutter}s · ISO ${settings.iso}`} exposure={exposure} />
        <div className="camera-controls">
          ...
          <button className="button primary-button capture-button" disabled={!hydrated} onClick={() => setFeedback(evaluateCyclistAttempt(settings, intention))}>Take photo</button>
...
function CyclistPreview({ shutter, motion, label, exposure = 0 }: { shutter: number; motion: ReturnType<typeof cyclistMotion>; label: string; exposure?: number }) {
